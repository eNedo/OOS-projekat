...
public class MainClass
{
    ...
    public int getFreeDirectoryBlock(RandomAccessFile x) throws IOException
    {
        x.seek(x.getFilePointer() + (long) RootHeader.ROOTHEADERSIZE);
        RootHeader test = new RootHeader();
        int sizeOfFile = test.getSizeOfMFTFiles();
        int sizeOfDir = test.getSizeOfMFTDirs();
        long freeSpace = ONEMB - (long) (sizeOfDir + sizeOfFile);
        int countBlocks = 0;
        if (freeSpace >= (long) 256)
        {
            while (true)
            {
                countBlocks++;
                byte temp = x.readByte();
                // filePosition.seek(filePosition.getFilePointer() - 1);
                if ((temp == 0) || (countBlocks * 256 == test.getSizeOfMFTDirs()))
                {
                    return countBlocks;
                } else
                {
                    x.seek(x.getFilePointer() + 255);
                }
            }
        }
        return -1;//no free space or everything in use
    }
}