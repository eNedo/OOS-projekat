...
public class FileHeader
{
    ...
    public int writeFiletoMFTheader(RootHeader rootheader, RandomAccessFile file, String ojsa)
    {
        try
        {
            ...
            if ((rootheader.getsizeOfMFTFileHeaders() + rootheader.getSizeOfMFTFiles()) == 0)
                file.seek(file.getFilePointer() - 64);
            else
            {
                for (int i = (rootheader.getsizeOfMFTFileHeaders() + rootheader.getSizeOfMFTFiles()); i > 0; i--)
                {
                    ...
                    if (isMFTfile == 1 && allocationflag == 1 && mftflag == 1)
                    {
                        helpcounter++;
                        file.seek(file.getFilePointer() - 66);
                        break;
                    }
                    if (isMFTfile == 0 && allocationflag == 1)
                    {
                        helpcounter++;
